...
def get_cached_ai_response(cache_key):
    conn = get_connection()
    row = conn.execute(
        "SELECT response_text, created_at FROM ai_cache WHERE cache_key = ?",
        (cache_key,),
    ).fetchone()
    conn.close()
    if row:
...
def get_analytics_summary():
    ...
    total_itineraries = conn.execute(
        "SELECT COUNT(*) FROM saved_itineraries"
    ).fetchone()[0]
    ai_itineraries = conn.execute(
        "SELECT COUNT(*) FROM saved_itineraries WHERE ai_enhanced = 1"
    ).fetchone()[0]
    ...
    return {
        ...
        "top_events": [
            {"type": r["event_type"], "count": r["cnt"]} for r in popular_events
        ],
    }
